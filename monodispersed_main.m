...
T=zeros(length_lamda,ne_repeat);
R=zeros(length_lamda,ne_repeat);
for tekrar=1:ne_repeat
for i=1:length_lamda
    n_particle = 1.5;   % real refractive index of the pigment
    k_particle = 0;     % imaginary refractive index of the pigment
    n_medium = 1;


    x_s = 2*pi*particle_radius/lamda(i);
    if (x_s < 8)
...
%     display particles
    ...
    sonuclar=[T(i,tekrar),R(i,tekrar),cyl_fv_sonuc,lamda(i)];
    writematrix(sonuclar,'results.txt','WriteMode','append')
